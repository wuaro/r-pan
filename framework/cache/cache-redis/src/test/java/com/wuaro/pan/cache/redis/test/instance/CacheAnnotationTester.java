package com.wuaro.pan.cache.redis.test.instance;
...
public class CacheAnnotationTester {
     ...
     * 注解：
     *      1. @Cacheable
     *          是 Spring Framework 提供的一种用于声明方法支持缓存功能的注解。
     *          通过在方法上添加 @Cacheable 注解，可以告诉 Spring，在调用该方法时，先检查缓存中是否已存在对应的结果，
     *          如果存在，则直接返回缓存中的结果，而不执行方法体内的逻辑；
     *          如果缓存中不存在对应的结果，则执行方法体内的逻辑，并将方法的返回值缓存起来。
     *          关于sync属性：
     *              如果设置为true，则出现多个线程同时缓存未命中，需要击穿缓存去数据库查询时，只会放一个线程进入数据库查询，其他线程需要阻塞
